...
class ProjectOverview(TaskSet):
    ...
    def on_start(self):
        r = self.client.get("/project").content
        decoded_data = r.decode('utf-8')
        projects = re.search('{"projects":(\\[.*\\])', decoded_data, re.MULTILINE).group(1)
        projects = '{"projects":'+projects+"}"
        self.projects = json.loads(projects)
        assert len(self.projects) > 0, "No project founds create some!"
        self.csrf_token = csrf.find_in_page(r)
